...
#ifndef FIRESTORE_CORE_INCLUDE_FIREBASE_FIRESTORE_SOURCE_H_
#define FIRESTORE_CORE_INCLUDE_FIREBASE_FIRESTORE_SOURCE_H_

namespace firebase {
namespace firestore {

/**
 * Configures the behavior of {@code Get()} calls on {@link DocumentReference}
 * and {@link Query}. By providing a Source value, these methods can be
 * configured to fetch results only from the server, only from the local cache,
 * or attempt to fetch results from the server and fall back to the cache (which
 * is the default).
 */
enum class Source {
  /**
   ...
   * satisfy the {@code get()} call, {@link DocumentReference#get()} will return
   * an error and {@link Query#get()} will return an empty
...
#endif  // FIRESTORE_CORE_INCLUDE_FIREBASE_FIRESTORE_SOURCE_H_
